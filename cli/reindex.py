...
def enviar_para_qdrant(
    ...
    try:
        ...
        if dry_run:
            print(f"[DRY RUN] ID gerado: {doc_id} para: {metadata.get('caminho_absoluto')}")
            return doc_id
...
def processar_arquivo(
    ...
    try:
        ...
        extensoes_ignoradas = {'.pyc', '.pyo', '.so', '.o', '.a', '.lib', '.dll', '.exe', 
                              '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff', '.webp',
                              '.mp3', '.mp4', '.avi', '.mov', '.flv', '.mkv', '.zip', 
                              '.tar', '.gz', '.rar', '.7z'}
...
def main():
    ...
    parser.add_argument(
        "--path",
        required=True, 
        help="Caminho para arquivo ou diretório a ser indexado"
    )
    ...
    try:
        if os.path.isfile(args.path):
            ...
            print(f"Processamento concluído. {len(sucesso)} de {len(resultados)} arquivos indexados.")
