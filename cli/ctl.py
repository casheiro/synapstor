...
def is_running():
    ...
    try:
        ...
        return "synapstor" in process.name().lower() or "python" in process.name().lower()
...
def start_server(args):
    ...
    if args.configure:
        ...
        if not configurador.configurar():
            logger.error("❌ Falha ao configurar o Synapstor. O servidor não será iniciado.")
            return 1
    ...
    try:
        ...
        if is_running():
            ...
            logger.error("❌ Servidor falhou ao iniciar. Verifique os logs para mais detalhes.")
            return 1
...
def stop_server():
    ...
    try:
        ...
        for _ in range(max_wait):
            ...
            logger.warning("⚠️ O servidor não respondeu ao sinal SIGTERM, enviando SIGKILL...")
            try:
    ...
    try:
        ...
        if args.follow:
            if os.name == 'nt':  # Windows
                ...
                try:
                    process = subprocess.Popen(
                        ["powershell.exe", "-Command", f"Get-Content -Path '{LOG_FILE}' -Wait"],
                        stdout=sys.stdout
                    )
                    process.wait()
            ...
            if args.tail > 0:
                ...
                if os.name == 'nt':  # Windows
                    process = subprocess.Popen(
                        ["powershell.exe", "-Command", f"Get-Content -Path '{LOG_FILE}' -Tail {args.tail}"],
                        stdout=sys.stdout
                    )
                else:  # Linux/macOS
                    process = subprocess.Popen(
                        ["tail", f"-n{args.tail}", LOG_FILE],
                        stdout=sys.stdout
                    )
                process.wait()
            else:
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Gerencia o servidor Synapstor como um serviço")
    subparsers = parser.add_subparsers(dest="comando", help="Comandos disponíveis")
    
    # Subcomando para iniciar o servidor
    start_parser = subparsers.add_parser("start", help="Inicia o servidor em segundo plano")
    ...
    log_parser.add_argument(
        "-f", "--follow", 
        action="store_true",
        help="Acompanha os logs em tempo real"
    )
    log_parser.add_argument(
        "-n", "--tail", 
        type=int, 
    ...
    reindex_parser.add_argument(
        "--project", 
        required=True,
        help="Nome do projeto a ser indexado"
    )
    reindex_parser.add_argument(
        "--path", 
        help="Caminho do projeto a ser indexado"
    )
    reindex_parser.add_argument(
        "--env-file", 
        help="Caminho para o arquivo .env"
    ...
    indexer_parser = subparsers.add_parser("indexer", help="Executa o indexador do Synapstor")
    ...
    indexer_parser.add_argument(
        "--dry-run", 
        action="store_true",
        help="Simula a indexação sem enviar ao Qdrant"
    )
