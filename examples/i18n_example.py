...
def exemplo_configuracao_ambiente():
    ...
    for codigo, nome in ambientes:
        print(f"=== {nome} (SYNAPSTOR_LANGUAGE={codigo}) ===")
        
        # Simular variável de ambiente
        os.environ["SYNAPSTOR_LANGUAGE"] = codigo
        
        # Configurar idioma baseado no ambiente
...
def exemplo_modo_synapstor():
    ...
    for lang, lang_name in [(Language.PORTUGUESE, "Português"), (Language.ENGLISH, "English")]:
        print(f"=== {lang_name} ===")
        set_language(lang)
        ...
        print(f"Documentos: {translator.translate('modo_synapstor.total_documents', count=3)}")
...
async def exemplo_plugin_async():
    ...
    async def simular_plugin_function(tema: str, lang: Language):
        ...
        print(f"[{lang.value.upper()}] {translator.translate('modo_synapstor.debug.starting', theme=tema)}")
        
        # Simular delay de processamento
        await asyncio.sleep(0.1)
        
        # Simular consulta RAG
        print(f"[{lang.value.upper()}] {translator.translate('modo_synapstor.debug.consulting_rag')}")
        await asyncio.sleep(0.1)
        
        # Simular construção de prompt
        print(f"[{lang.value.upper()}] {translator.translate('modo_synapstor.debug.building_prompt', count=4)}")
        await asyncio.sleep(0.1)
        
        # Simular sucesso
        print(f"[{lang.value.upper()}] {translator.translate('modo_synapstor.debug.success')}")
...
def exemplo_validacao():
    ...
    for lang, lang_name in [(Language.PORTUGUESE, "Português"), (Language.ENGLISH, "English")]:
        ...
        if missing_keys:
            ...
            print(f"  🎉 Todas as chaves testadas estão presentes!")
        print()
