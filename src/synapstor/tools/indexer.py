...
try:
    ...
    print(
        "⚠️ Módulo synapstor.utils não encontrado, usando versão interna de gerar_id_determinista"
    )
...
class IndexadorDireto:
    ...
    def __init__(
        ...
        try:
            ...
            # Pega a URL e API key do .env se não fornecidas
            if not qdrant_url:
            ...
            print(f"✅ Conectado ao servidor Qdrant: {qdrant_url}")
        except Exception as e:
            print(f"❌ Falha ao conectar ao Qdrant: {e}")
            raise ValueError(f"Não foi possível conectar ao servidor Qdrant: {e}")
        ...
        if not self.caminho_projeto.exists() or not self.caminho_projeto.is_dir():
            raise ValueError(
                f"O caminho do projeto não existe ou não é um diretório: {caminho_projeto}"
            )
    ...
    def _garantir_colecao(self):
        ...
        try:
            ...
            if not collection_exists:
                ...
                print(
                    f"✅ Coleção '{self.collection_name}' criada com sucesso usando nome de vetor '{self.vector_name}'!"
                )
            else:
    ...
    def _obter_configuracao_colecao(self):
        ...
        try:
            ...
            print(
                f"⚠️ Não foi possível determinar o nome do vetor. Usando padrão: {self.vector_name}"
            )
            ...
            self.vector_name = "fast-all-minilm-l6-v2"
            print(
                f"⚠️ Erro ao obter configuração da coleção: {e}. Usando nome de vetor padrão: {self.vector_name}"
            )
                ...
                if hasattr(colecao_info.config.params, "vectors"):
                    ...
                    if isinstance(vectors_config, dict):
                        for vector_name, vector_params in vectors_config.items():
                            print(
                                f"    - {vector_name}: tamanho={getattr(vector_params, 'size', 'N/A')}, distância={getattr(vector_params, 'distance', 'N/A')}"
                            )
                    else:
    ...
    def _enviar_para_qdrant(self, conteudo: str, metadata: Dict[str, Any]) -> bool:
        ...
        try:
            # Importação local para evitar erros de tipos
    ...
    def _formatar_tamanho(self, tamanho_bytes: int) -> str:
        ...
        tamanho_formatado = float(tamanho_bytes)  # Converte para float explicitamente
    ...
    def indexar(self) -> bool:
        """Indexa todos os arquivos do projeto recursivamente"""
        try:
            # Estatísticas
            total_arquivos = 0
            arquivos_para_processar = []

            # Barra de progresso para descoberta de arquivos
            print(f"🔍 Descobrindo arquivos em: {self.caminho_projeto}")
            ...
            for root, _, files in os.walk(self.caminho_projeto):
                ...
                for file in files:
                    ...
                    # Filtra apenas arquivos de texto que não devem ser ignorados pelo gitignore
                    if (
                    ...
                    with concurrent.futures.ThreadPoolExecutor(
                        ...
                        def processar_com_progresso(caminho):
                            ...
                            pbar.set_description(
                                f"Indexando: {rel_path[:40]}{'...' if len(rel_path) > 40 else ''}"
                            )
            ...
            print(
                f"   Arquivos processáveis: {total_para_processar} ({(total_para_processar/total_arquivos)*100:.1f}%)"
            )
            print(
                f"   Arquivos indexados: {self.arquivos_indexados} ({(self.arquivos_indexados/total_para_processar)*100:.1f}%)"
            )
...
def main():
    ...
    parser.add_argument(
        "--query", "-q", help="Opcional: realiza uma busca após a indexação"
    )
    parser.add_argument(
        "--workers",
        "-w",
