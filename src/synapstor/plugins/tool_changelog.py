...
def _obter_commits(desde: Optional[str] = None, ate: Optional[str] = None) -> List[Dict[str, Any]]:
    """
...
def _formatar_changelog(commits: List[Dict[str, Any]], versao: str) -> str:
    ...
    for tipo in ["feat", "fix", "perf"]:
        if tipo in commits_por_tipo and commits_por_tipo[tipo]:
            ...
            secoes.append(SECTION_TEMPLATE.format(
                tipo=titulo,
                itens=itens.strip()
            ))
    ...
    if breaking_changes:
        ...
        for commit in breaking_changes:
            escopo = f"**{commit['escopo']}**: " if commit['escopo'] else ""
            mensagem = commit["breaking_desc"] if commit["breaking_desc"] else commit["mensagem"]
            hash_commit = commit["hash"]
...
async def gerar_changelog(
    ...
    """
    await ctx.debug(f"Gerando changelog desde: {desde}, até: {ate}, arquivo: {arquivo_saida}")
    ...
    try:
        ...
        if not incluir_todos:
            commits = [c for c in commits if c["tipo"] in TIPOS_COMMITS or c["tipo"] == "outros"]
...
async def verificar_commits(
    ...
    try:
        ...
        resultados.append(f"Commits conformes: {conformes} ({int(conformes/total*100)}%)")
        resultados.append(f"Commits não conformes: {nao_conformes} ({int(nao_conformes/total*100)}%)")
        ...
        if detalhado:
            ...
            for commit in commits:
                ...
                resultados.append(f"{conforme} {hash_commit}: {tipo}{escopo}: {mensagem}")
